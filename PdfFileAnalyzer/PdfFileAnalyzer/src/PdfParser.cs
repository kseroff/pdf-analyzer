...
namespace PdfFileAnalyzer
	...
	public class PdfParser
		...
		internal int LastParsedGen = 0;

		internal const int EOF = -1;

		internal PdfParser( PdfReader Reader, bool ContentsStream )
			{
			this.Reader = Reader;
		...
		public int ParseObjectRefNo()
		{
			...
			// Читаем номер объекта
			StringBuilder NextItem = new StringBuilder(((char)NextChar).ToString());
			while ((NextChar = ReadChar()) != EOF && !PdfBase.IsDelimiter(NextChar))
				NextItem.Append((char)NextChar);

			int ObjNo;
			if (!int.TryParse(NextItem.ToString(), out ObjNo) || ObjNo <= 0) return 0;

			// Пропускаем пробелы
			if (!PdfBase.IsWhiteSpace(NextChar)) return 0;
			while ((NextChar = ReadChar()) != EOF && PdfBase.IsWhiteSpace(NextChar)) ;

			// Читаем номер поколения (Generation)
			StringBuilder GenItem = new StringBuilder();
			while (NextChar != EOF && char.IsDigit((char)NextChar))
			{
				GenItem.Append((char)NextChar);
				NextChar = ReadChar();
			}

			int Generation;
			if (!int.TryParse(GenItem.ToString(), out Generation) || Generation < 0)
				return 0;

			// Пропускаем пробелы и "obj"
			if (!PdfBase.IsWhiteSpace(NextChar)) return 0;
			while ((NextChar = ReadChar()) != EOF && PdfBase.IsWhiteSpace(NextChar)) ;
			...
			// Сохраняем номер поколения
			this.LastParsedGen = Generation;
		...
		public PdfBase ParseNextItem()
			...
			if(int.TryParse(Token, out int IntVal))
				...
				if(!ContentsStream && IntVal > 0 && TestReference())
					return new PdfReference(IntVal, LastParsedGen);
...
//					if(NextChar == '\n' || NextChar == '\r' && ReadChar() == '\n') return new PdfKeyword(KeyWord.Stream);
		...
		internal PdfDictionary ParseDictionary(bool InlineImage)
			{
